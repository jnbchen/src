...
class can_collector(object):
    def __init__(self):
        ...
        self.pub_1 = rospy.Publisher('/can_dumper_node/can1_data', RadarArray, queue_size=1)
        self.pub_2 = rospy.Publisher('/can_dumper_node/can2_data', RadarArray, queue_size=1)
        self.pub_5 = rospy.Publisher('/can_dumper_node/can5_data', RadarArray, queue_size=1)

        config             = rospy.get_param('~device_config_file', './')
        self.interval_     = rospy.get_param('~collect_interval', 10)
        self.broadcast_    = rospy.get_param('~broadcast_time', 30)
        ...
        for i in range(len(self.buffer_)):
            ...
            if (r['channel'] == 'can1' or r['channel'] == 'can2') and r['id'] in self.list_ssr_:
                if r['TrackSt_RollCount'] > 2:
                    ...
                    radar.pose.x = r['Track_LatDispl']
                    radar.pose.y = r['Track_LongDispl']
                ...
                if id not in buffer.keys():
                    ...
                    if len(buffer[id].keys()) == 24:
                        res = buffer[id]
                        if res['Obj_ProbOfExist'] > 3:
                            radar.id = res['Obj_ID']
                            radar.radar_type = 'ContiARS408'
                            radar.is_object = True
                            radar.rcs_value = res['Obj_RCS']
                            radar.pose.x = res['Obj_DistLat']
                            radar.pose.y = res['Obj_DistLong']
                            radar.pose.theta = res['Obj_OrientationAngle']
                            ...
                            radar.color.r = self.switch(radar.classification).get('r')
                            radar.color.g = self.switch(radar.classification).get('g')
                            radar.color.b = self.switch(radar.classification).get('b')
                            radar.color.a = self.switch(radar.classification).get('a')
                            #print(radar)
    ...
    def start(self):
        self.job.enter(self.interval_/1000.0, 1, self.collect, argument=())
        self.job.enter(self.broadcast_/1000.0, 1, self.broadcast, argument=())
        self.job.run()
